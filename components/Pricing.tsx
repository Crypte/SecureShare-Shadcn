...
const PricingHeader = ({
  ...
  <section className="text-center">
    ...
    <p className="text-xl pt-1 text-muted-foreground">{subtitle}</p>
    <br />
  </section>
);
  ...
  >
      ...
      <CardHeader className="pb-8 pt-4">
        {isYearly && yearlyPrice && monthlyPrice ? (
          <div className="flex justify-between">
            ...
            >
              Sauvez €{monthlyPrice * 12 - yearlyPrice}
            </div>
          </div>
        ) : (
          <CardTitle className="text-zinc-700 dark:text-zinc-300 text-lg">
        ...
        <div className="flex gap-0.5">
          ...
          <span className="flex flex-col justify-end text-sm mb-1">
            {yearlyPrice && isYearly ? "/an" : monthlyPrice ? "/mois" : null}
          </span>
        </div>
    ...
    <CardFooter className="mt-2">
      <Button className="relative inline-flex w-full items-center justify-center rounded-md bg-black text-white dark:bg-white px-6 font-medium  dark:text-black transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
        ...
        <Link href={linkurl}>{actionLabel}</Link>
      </Button>
    </CardFooter>
  </Card>
);
  ...
  const plans = [
    {
      ...
      monthlyPrice: 0,
      yearlyPrice: 0,
      description: "Chiffrement local de vos fichiers",
      features: [
        "Tous les formats de fichier",
        "Deux façons de les sécuriser",
        "Un protocol de chiffrement fort",
      ],
